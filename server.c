...
int main (int argc, char *argv[])
{
    ...
    for (int i = 1; ; i++) {
        ...
        // next message from client should have this sequence number
        ...
        while(1) {
            ...
            if (n > 0) {
                ...
                // check sequence number. If recvpkt.seqnum == expected seqnum (cliSeqNum ??)
                //      send packet payload to file
                //      send ack seqnum + body size
                //      change next spected seqnum
                ...
                if (recvpkt.fin) {
                    ...
                    //expected packet: write payload to file, modify cliSeqNum, build ack packet, print ack packet, send ack packet
                    ...
                    //records sequence number of last packet that we sent an ACK for
                    prevAck = cliSeqNum;
                    isDup = 0;

                } else {
                    //non-expected packet: do not write to file, build ack packet, do not modify cliSeqNum, print ack packet, send ack packet
                    seqNum = recvpkt.acknum;
                    int dup = 0;
                    int ack = 1;

                    //if this packet has same sequence # as seq # of previously acked packet, this will be a DUP ACK
                    if(cliSeqNum == prevAck || isDup){
                        dup = 1;
                        ack = 0;
                    }

                    buildPkt(&ackpkt, seqNum, cliSeqNum, 0, 0, ack, dup, 0, NULL);
                    printSend(&ackpkt, 0);
                    sendto(sockfd, &ackpkt, PKT_SIZE, 0, (struct sockaddr*) &cliaddr, cliaddrlen);
                    isDup = 1;
                }
